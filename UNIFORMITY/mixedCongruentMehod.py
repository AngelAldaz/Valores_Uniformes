...
def modular_sequence(a,c,m,X0):
  ...
  while True:
    ...
    cont+=1
    n.append(cont)
    XnMod.append(((Xn[n[cont-1]-1]*a)+c)%m)
    Xn.append(XnMod[cont-1])
    rDR.append(f"{((Xn[n[cont-1]-1]*a)+c)}/{m} = {int(((Xn[n[cont-1]-1]*a)+c)/m)} + {Xn[cont]}/{m}")
...
def main():
  ...
  cont, n, Xn, rDR, XnMod, numAl = modular_sequence(a,c,m,X0)
